...
@ImgAlpha_ON:    (User define) If true: apply imgbuf alpha.
                 ...
                 color index at all, then it will speed up IMGBUF displaying.
...
/*---------------------------------------------------------------------------------------
Display raster data of a frame or block of GIF image.
The block raster data will first write to Simgbuf, which hooked as the whole GIF canvas,
...
-------------------------------------------------------------------------------------------*/
static void egi_gif_rasterWriteFB( FBDEV *fbdev, EGI_IMGBUF *Simgbuf, int Disposal_Mode, int x0, int y0,
...
//	Is_ImgColorMap=false;
	...
	for( j=0; j< ImageData->ExtensionBlockCount; j++ )
	{
     ...
     /* Write gif frame/block to FB, ALWAYS after a control block! */
      egi_gif_rasterWriteFB( fbdev, egif->Simgbuf, Disposal_Mode, x0, y0,  /*  Simgbuf, Disposal_Mode, x0, y0, */
     ...
     egif->ImageCount++;
     if( loop_gif && egif->ImageCount > egif->ImageTotal-1 )
		egif->ImageCount=0;
