...
TODO:
 ...
 3. For big GIF file, DO NOT try to use slurp meth, it needs large mem space!
...
while(1) {	/////////////////////    LOOP TEST    ///////////////////////
	...
	printf("\nGet ImageData[%d] ...\n", i);
        ...
        printf("image: off(%d,%d) size(%dx%d) ", offx,offy,Width, Height);

        /* Get extension block */
        if( ImageData->ExtensionBlockCount > 0 )
		ExtBlock=&ImageData->ExtensionBlocks[0];
	else
		ExtBlock=NULL;

	/*   --- Parse extension before image ---  */
	if (  ExtBlock != NULL && ExtBlock->Function == GRAPHICS_EXT_FUNC_CODE )
        {
