...
int main(int argc, char **argv)
{
	...
	write_tag(24+0, 10, 12, 20, 20, (const wchar_t *)L"符", WEGI_COLOR_BLUE );  /* keynum, x0,y0,fw,fh, pustr */
        write_tag(24+1, 0, 16, 12, 12, (const wchar_t *)L"SHIFT", WEGI_COLOR_GRAY3 );  /* keynum, x0,y0,fw,fh, pustr */
...
while(1) {
	...
	/* wait for touch event */
 	if( egi_touch_timeWait_press(3, &touch_data)==0 ) {
			...
			/* If control keys */
			switch(6*nrow+ncol) {
				case 24:		/* page shift key in Alphabets page */
					if(keypage==1) {
						keypage=0;
						keycase=0;
						Is_ControlKey=true;
			...
			/* wait for release */
			while( egi_touch_timeWait_release(1, &touch_data) !=0 );
	}
} /* End while() */
