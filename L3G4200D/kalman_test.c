...
/*----------------  System Matrix  ------------------
Mat_Y[]: the stat matrix of concerning variables (updated)
Mat_Yp[]: predicted of Mat_Yp[]
Mat_F[]: the state-transition matrix
Mat_H[]: the observation matrix
Mat_Q[]: the covariance matrix of the process noise
Mat_R[]: the covarinace matrix of the observation noise
Mat_P[]: the state_estimation error covaraince matrix (updated)
Mat_Pp[]: predicted of Mat_P[]
Mat_K[]: Kalman Gain matrix
----------------------------------------------------*/
...
float MatF[3*3]= //transition matrix
{
  1, 1, 0,
  0, 1, 1,
  0, 0, 1
...
float MatK[3*1]={0}; //for temp. buff
...
//-------------- test -------------------------
        ...
        for(k=0; k<n; k++)
	{
		...
		printf("--------- k=%d ---------\n",k);
		//-----Predict(priori) state:  Yp = F*Y -----(3,1) = (3,3)*(3,1)
		Matrix_Multiply(&Mat_F,&Mat_Y,&Mat_Yp);
		printf("Mat_Yp=\n");
		Matrix_Print(Mat_Yp);

		//-----Predict(priori) state covariance:  Pp = F*P*F'+Q ----- (3,3)=(3,3)*(3,3)*(3,3)'+(3,3)
		Matrix_Add (
...
//		Matrix_Print(Mat_1X1C);
		...
		//-----Update(posteriori) state covariance:  P = (I-K*H)*Pp   ---- (3,1) = ( (3,3)-(3,1)*(1,3) )*(3,1)
