...
//--------------------
uint8_t readoledRegVal(uint8_t reg_addr)
{
    int ret;
    struct i2c_rdwr_ioctl_data i2c_iodata;
    ...
    i2c_iodata.msgs[0].len=1;
    i2c_iodata.msgs[0].addr=g_u8oledAddr;
    i2c_iodata.msgs[0].flags=0; //write
    i2c_iodata.msgs[0].buf[0]=reg_addr;

    i2c_iodata.msgs[1].len=1;
    i2c_iodata.msgs[1].addr=g_u8oledAddr;
    i2c_iodata.msgs[1].flags=I2C_M_RD; //read reg
    i2c_iodata.msgs[1].buf=(unsigned char*)malloc(1);
    i2c_iodata.msgs[1].buf[0]=0;//clear it first
    ...
    return i2c_iodata.msgs[1].buf[0];
...
/*
	//----set mode---
	//-----set as horizontal addressing mode -----
        sendCmdOled(0x20);  //set memory addressing mode
        sendCmdOled(0x00); //[1:0]=00b-horizontal 01b-veritacal 10b-page addressing mode(RESET)
	//---set column addr. for horizontal mode
	sendCmdOled(0x21);
	sendCmdOled(start_column);//column start
	sendCmdOled(start_column+7);//column end, !!!! 8x16 for one line only!!!
