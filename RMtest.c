...
char strf[STRBUFF]; //--for file name
 ...
 /* ----------- find out all BMP files in specified path -------- */
   ...
   if(BMP_file_total == 0){
     printf("----- No BMP file found! -----\n");
     return 1; }
   else
	printf("-----Totally %d BMP files found!------\n",BMP_file_total);
 ...
 //------ !!! Warning SPI mode shoul be set in spi.h properly --------- 
   printf("-----start opening SPI device ----\n");
   SPI_Open();
   usleep(500000);
   printf("----start mmap for pin------\n");
   setPinMmap();
   usleep(500000);
...
// --------------------   Init LCD   ---------------------------- 
   printf("----start LCD_HD_reset()------\n");
    usleep(500000);
   LCD_HD_reset();
//   usleep(500000);
//   printf("----start LCD_INIT_RM68140------\n");
//   LCD_INIT_RM68140();
