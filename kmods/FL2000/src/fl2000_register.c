	...
	uint32_t* data)
{
	...
	printk("======== In fl2000_reg_write() start: fl2000_i2c_xfer()....\n");
	...
	uint32_t* data)
{
	...
	*data = 0;
	...
	printk("======== In fl2000_reg_read() start: fl2000_i2c_xfer()....\n");
	ret_val = fl2000_i2c_xfer(
		dev_ctx,
		VGA_MMIO_READ,
		offset,
	...
	uint32_t bit_offset)
{
	...
	printk("======== In fl2000_reg_bit_clear() start: fl2000_reg_read()....\n");
	status = fl2000_reg_read(dev_ctx, offset, &data);
	if (!status) {
		dbg_msg(TRACE_LEVEL_ERROR, DBG_HW,
			"fl2000_reg_read failed.");
	...
	printk("======== In fl2000_reg_bit_clear() start: fl2000_reg_write()....\n");
	status = fl2000_reg_write(dev_ctx, offset, &data);
	if (!status) {
		dbg_msg(TRACE_LEVEL_ERROR, DBG_HW,
			"fl2000_reg_write failed.");
