...
int create_fifo_thd(void *routine,int dwPriority)
{
	pthread_attr_t Attr;
...
struct sched_param my_param;
	...
	int ret=0;
	ret=create_fifo_thd(thd1,11); //thread1 FIFO priority 11
	if(0==ret){
	...
	ret=create_fifo_thd(thd2,maxpi); //thread1 FIFO priority 
	if(0==ret){
