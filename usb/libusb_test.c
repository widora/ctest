...
int main(void)
{
   ...
   unsigned char data[4]={0}; // wMaxPacketSize=4 bytes defined in USB 
...
--------------------------------------------------------------------------------*/
 ...
 while(1)
 {
    ...
    r=libusb_interrupt_transfer(dev_handle,0x01,data,1, &cnt, 200); //0x00:host->dev, 0x80:dev->host
    if(!r)
	perror("libusb_interrupt_transfer() EP1-Out error");
    printf("%d bytes data have been transferred to EP01 .\n",cnt);
    //---- read from EP 1 IN  ---
